...
void Round::readImage(){
    tRound->LoadImagePNG("eat", rR);
    tRound->LoadImagePNG("eatClick", rR);
    tRound->LoadImagePNG("pon", rR);
    tRound->LoadImagePNG("ponClick", rR);
    tRound->LoadImagePNG("gang", rR);
    tRound->LoadImagePNG("gangClick", rR);
    tRound->LoadImagePNG("ting", rR);
    tRound->LoadImagePNG("tingClick", rR);
    tRound->LoadImagePNG("hu", rR);
    tRound->LoadImagePNG("huClick", rR);
    tRound->LoadImagePNG("error", rR);
    tRound->LoadImagePNG("button", rR);
}
...
void Round::updateAI() {
    ...
    if(!haveWinner() && !failToFindWinner){
        ...
        justAction = false;
        cout << "問題 2: player" << playerNow + 1 << " -- 聽了: " << playerList[playerNow]<< endl;
